...
$result = $conn->query("SELECT p.id_cuti, p.tanggal, p.alasan, p.idpeg, g.nama, p.daritgl, p.sampaitgl, p.lamacuti, p.ditetapkan, p.pembuat_surat FROM cuti p JOIN pegawai g ON p.idpeg = g.idpeg ORDER BY p.id_cuti DESC");
...
$urut = 1;
if ($latestid) {
    $latestNumber = (int) substr($latestid['id_cuti'], 8);  // Ambil 3 digit terakhir dari id_peringatan
    $urut = $latestNumber + 1;
}
$newid = 'C' . date('Y') . date('m') . str_pad($urut, 3, '0', STR_PAD_LEFT);
                            ...
                            <tbody>
                                ...
                                if ($result && $result->num_rows > 0) {
                                    ...
                                    while ($cuti = $result->fetch_assoc()) {
                                        ...
                                        echo "<button class='btn btn-warning btn-sm edit-btn mr-1' data-bs-toggle='modal' data-bs-target='#editCutiModal' data-id='" . htmlspecialchars($cuti['id_cuti']) . "' data-tanggal='" . htmlspecialchars($cuti['tanggal']) . "' data-alasan='" . htmlspecialchars($cuti['alasan']) . "' data-ditetapkan='" . htmlspecialchars($cuti['ditetapkan']) . "' data-pembuat_surat='" . htmlspecialchars($cuti['pembuat_surat']) ."'><i class='fas fa-edit'></i> Edit</button>";
                                        echo "<button class='btn btn-danger btn-sm delete-btn mr-1' data-id='" . htmlspecialchars($cuti['id_cuti']) . "'><i class='fas fa-trash'></i> Delete</button>";
                                        echo "<button class='btn btn-success btn-sm print-btn' data-id='" . htmlspecialchars($cuti['id_cuti']) . "'><i class='fas fa-print'></i> Print</button>";
                                        echo "</div>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="add_cuti.php" method="post">
                    ...
                    <div class="mb-3">
                        <label for="sampaitgl" class="form-label">Sampai Tanggal</label>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="lamacuti" name="lamacuti" readonly>
                    </div>
                    <div class="mb-3">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="edit_cuti.php" method="post">
                    ...
                    <div class="mb-3">
                        <label for="edit_daritgl" class="form-label">Dari Tanggal</label>
                        <input type="date" class="form-control" id="edit_daritgl" name="daritgl" required>
                    </div>
                    <div class="mb-3">
                        <label for="edit_sampaitgl" class="form-label">Sampai Tanggal</label>
                        <input type="date" class="form-control" id="edit_sampaitgl" name="sampaitgl" required>
                    </div>
                    <div class="mb-3">
                        <label for="edit_lamacuti" class="form-label">Lama Cuti</label>
                        <input type="text" class="form-control" id="edit_lamacuti" name="lamacuti" readonly>
                    </div>
                    <div class="mb-3">
...
<script>
    document.getElementById('daritgl').addEventListener('change', hitungLamaCuti);
        document.getElementById('sampaitgl').addEventListener('change', hitungLamaCuti);

        function hitungLamaCuti() {
            const dariTgl = new Date(document.getElementById('daritgl').value);
            const sampaiTgl = new Date(document.getElementById('sampaitgl').value);

            if (dariTgl && sampaiTgl && sampaiTgl >= dariTgl) {
                const diffTime = Math.abs(sampaiTgl - dariTgl);
                const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1; // Hitung selisih hari + 1 untuk menghitung hari pertama
                document.getElementById('lamacuti').value = diffDays;
            } else {
    ...
    document.addEventListener('click', function (e) {
        ...
        const sampaitgl = e.target.getAttribute('data-sampaitgl');
        ...
        document.getElementById('edit_sampaitgl').value = sampaitgl;
