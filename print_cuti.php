...
$stmt2 = $conn->prepare("SELECT c.id_cuti, c.idpeg, c.ditetapkan, c.tanggal, c.daritgl, c.sampaitgl, c.lamacuti, c.alasan, c.pembuat_surat, p.nama 
                         FROM cuti AS c
                         JOIN pegawai AS p ON c.idpeg = p.idpeg
                         WHERE c.id_cuti = ? LIMIT 1");
...
$pdf->Cell(0, 10, 'Surat Keterangan Cuti', 0, 1, 'C');
$pdf->SetDrawColor(0, 0, 0); // Warna hitam
...
$pdf->Cell(0, 10, 'Surat keterangan cuti ini ditujukan kepada :', 0, 1, 'L');
$pdf->Ln(1);
...
$pdf->Cell($labelWidth, 10, 'Dari', 0, 0, 'L'); // Kolom label Nama
$pdf->Cell($labelWidth, 10, ':', 0, 0, 'L'); 
$pdf->MultiCell(0, $lineHeight, $daritgl, 0, 'L'); // MultiCell untuk teks dengan spasi 1.25
$pdf->Ln(2);

// Alasan peringatan dengan pengaturan spasi 1.25
$pdf->SetFont('Arial', '', 11);
$lineHeight = 6.0 * 1.25; // Line height untuk spasi 1.25
$pdf->Cell($labelWidth, 10, 'Sampai', 0, 0, 'L'); // Kolom label Nama
$pdf->Cell($labelWidth, 10, ':', 0, 0, 'L'); 
$pdf->MultiCell(0, $lineHeight, $sampaitgl, 0, 'L'); // MultiCell untuk teks dengan spasi 1.25
$pdf->Ln(2);

// Alasan peringatan dengan pengaturan spasi 1.25
$pdf->SetFont('Arial', '', 11);
$lineHeight = 6.0 * 1.25; // Line height untuk spasi 1.25
$pdf->Cell($labelWidth, 10, 'Lama', 0, 0, 'L'); // Kolom label Nama
$pdf->Cell($labelWidth, 10, ':', 0, 0, 'L'); 
$pdf->MultiCell(0, $lineHeight, $lamacuti, 0, 'L'); // MultiCell untuk teks dengan spasi 1.25
$pdf->Ln(2);
