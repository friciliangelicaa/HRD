...
$stmt2 = $conn->prepare("SELECT i.id_izin, i.idpeg, i.ditetapkan, i.tanggal, i.jam, i.alasan, i.pembuat_surat, p.nama 
                         FROM izin AS i
                         JOIN pegawai AS p ON i.idpeg = p.idpeg
                         WHERE i.id_izin = ? LIMIT 1");
...
$pdf->Cell(0, 10, 'Surat Keterangan Izin', 0, 1, 'C');
$pdf->SetDrawColor(0, 0, 0); // Warna hitam
...
$pdf->Cell(0, 10, 'Surat keterangan izin ini ditujukan kepada :', 0, 1, 'L');
$pdf->Ln(1);
...
$pdf->Cell($labelWidth, 10, ':', 0, 0, 'L'); 
$pdf->MultiCell(0, $lineHeight, $tanggal, 0, 'L'); // MultiCell untuk teks dengan spasi 1.25
$pdf->Ln(2);
...
$pdf->Cell($labelWidth, 10, ':', 0, 0, 'L'); 
$pdf->MultiCell(0, $lineHeight, $jam, 0, 'L'); // MultiCell untuk teks dengan spasi 1.25
$pdf->Ln(2);
